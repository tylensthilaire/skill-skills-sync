...
- **hash** — sha256 content hash of the skill folder at that ref, as printed
  by `install.py` (`install.py hash <folder>` recomputes it). This is what
  guarantees the file that runs is the file that was reviewed. Leave it out on
  first add; `install.py` prints the hash to pin. Refuse to run skills whose
  hash does not match.
